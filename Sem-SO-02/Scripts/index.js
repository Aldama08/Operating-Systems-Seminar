...
//Función para el boton de ingresar, abre la pestaña de los datos para procesos
...
function ObtenerNumeroProcesos(){
    ...
    if(validarNumero(procesos.value)){
       lote = [];
       lotes = [];
       numeroLotes =  redondear(procesos.value/5); //Obtenemos la cantidad de procesos para completar 1 lote
       numeroProcesos = procesos.value; //Guardamos la cantidad de procesos
       menuProceso(); //Mandamos a llamar a la función y desplegamos el bloque.
    }else{
...
/*Estructura dentro del marco con la leyenda proceso*/
    if(numeroProcesos > 0){
        divProceso.innerHTML = `
        <form class="formulario">
            <p>Lotes: ${numeroLotes}<p>
            <p> Procesos: ${numeroProcesos}</p>
            <fieldset class="formulario2e">
            ...
            <input class="boton" type="button" id="nuevoproceso" value="Ingresar">
        </form>
        `;
...
function validarOperador( operacion ){
    const regex = new RegExp('^[0-9]+[+-/%\*xs]{1,1}[0-9]+$'); //"s" como operador para los porcentajes
    return regex.test(operacion); // true o false
} 

function IsValidDivision(operacion){ //2/2
    const operador = operacion.match('[+-/%\*xs]{1,1}') //"s" como operador para los porcentajes
...
//Se pueden aceptar más lotes, pero puede llegar a fallar los contadores.
